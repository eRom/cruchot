...
const CORE_PROMPT_TEMPLATE = `Tu es l'assistant vocal d'une application desktop multi-LLM.
...
function buildStaticPrompt(): string {
  return CORE_PROMPT_TEMPLATE.replace('{{IDENTITY_PROMPT}}', getIdentityPrompt())
}
...
export function buildDynamicContext(): string {
  ...
  const convsXml = recentConvs
    .map(c => `  <conversation id="${c.id}" title="${c.title ?? 'Sans titre'}" model="${c.modelId ?? defaultModelId}" />`)
    .join('\n')

  const textModels = MODELS.filter(m => m.type === 'text')
  const modelsXml = textModels
    .map(m => `  <model id="${m.providerId}::${m.id}" name="${m.displayName}" provider="${m.providerId}" />`)
    .join('\n')
