...
Electron 35 + React 19 + TypeScript 5.7 + Tailwind CSS 4 + shadcn/ui + better-sqlite3 + Drizzle ORM + Zustand + Vercel AI SDK 6 (`ai@^6.0.116`) + `@ai-sdk/mcp` + Qdrant v1.17 (embedded) + `@huggingface/transformers` + `onnxruntime-node` + Vite standalone (remote-web SPA)
...
- SQLite WAL + FTS5, 19 tables (+ `vector_sync_state` S33)
