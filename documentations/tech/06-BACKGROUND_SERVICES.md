...
```
src/main/live/
  live-plugin.interface.ts     # Interface LivePlugin + types partagés (VoiceOption, LiveStatus…)
  ...
  plugins/
    ...
    openai/
      openai-live.plugin.ts    # OpenAILivePlugin (transport WebSocket natif ws)
      openai-live-tools.ts     # Outils OpenAI-specific + convertisseur CoreTool → OpenAI format
```
...
- `getAvailableVoices()` — liste des voix TTS supportées par le plugin (`VoiceOption[]`)
- Callbacks injectés par l'Engine avant `connect()` : `onAudio`, `onToolCall`, `onStatusChange`, `onTranscript`, `onError`

**`VoiceOption`** — interface partagée pour les voix de tous les plugins :
```typescript
interface VoiceOption { id: string; name: string; description: string }
```
La voix sélectionnée est persistée par plugin dans `settings.liveVoiceId` (format `providerId::voiceId`) et rechargée au démarrage de l'app.
...
- `getAvailablePlugins()` — liste tous les plugins avec disponibilité, capacités et voix disponibles
- `getVoiceForPlugin(plugin)` — lit `multi-llm:live-voice-<providerId>` depuis DB ; fallback sur la première voix du plugin
...
### 6.2b Plugin OpenAI Realtime (`openai-live.plugin.ts`)

Transport : **WebSocket natif** (`ws` package) sur `wss://api.openai.com/v1/realtime?model=gpt-realtime`.

Contrainte audio : l'app capture le micro à **16 kHz**, mais l'API OpenAI Realtime attend du **24 kHz**. Le plugin effectue un ré-échantillonnage par interpolation linéaire (ratio 1.5) côté main process dans la fonction `resample16to24()` avant d'envoyer chaque buffer.

Config session (message `session.update` à l'ouverture) :
- `output_modalities: ['audio']` — réponse audio uniquement
- `audio.input.turn_detection: { type: 'semantic_vad' }` — VAD sémantique
- `audio.input.format: { type: 'audio/pcm', rate: 24000 }` — PCM 24 kHz en entrée
- `audio.output.format: { type: 'audio/pcm', rate: 24000 }` + `voice` — voix configurable
- `tools` — 13 outils core convertis au format OpenAI via `convertToolsToOpenAI()` + outils plugin
- `tool_choice: 'auto'`

Particularité function calling : après chaque `sendToolResponse()`, le plugin doit envoyer explicitement `{ type: 'response.create' }` — OpenAI Realtime ne relance pas automatiquement la réponse après un tool output (contrairement à Gemini).

Gestion des interruptions : `speech_started` déclenche `response.cancel` + `input_audio_buffer.clear` **uniquement si** `isResponseActive === true`. Cela évite des cancel spurieux en début de tour (quand l'utilisateur commence à parler avant toute réponse).

Capacités :
- `supportsScreenShare()` → `false` (non supporté par l'API OpenAI Realtime)
- `buildFinalPrompt()` → identité (pas d'enrichissement provider-specific)
- 10 voix disponibles : Marin, Cedar, Alloy, Ash, Ballad, Coral, Echo, Sage, Shimmer, Verse

### 6.3 Cycle de vie et états

7 statuts possibles (`LiveStatus`) : `off` → `connecting` → `connected` → `listening` → `speaking` (+ `dormant` + `error` + `interrupted`).
...
**`GeminiLivePlugin` — méthodes screen share (capacité opt-in) :**
...
> Le plugin OpenAI Realtime ne supporte pas le screen sharing (`supportsScreenShare() → false`). L'icône monitor dans la NotchBar n'est affichée que si le plugin actif retourne `true`.
