...
15. [Decisions (questions tranchees)](#15-decisions-questions-tranchees)
...
## 15. Decisions (questions tranchees)

> Questions ouvertes resolues le 2026-03-13 apres exploration du codebase.

| # | Question | Decision | Justification |
|---|----------|----------|---------------|
| Q1 | Distribution binaire Qdrant | **C — Bundle prod + download dev** | `extraResources` contient deja `drizzle/`, ajouter `vendor/qdrant/` est trivial. `scripts/download-qdrant.sh` pour le dev. Fallback download si binaire absent. +50 MB acceptable pour du desktop. |
| Q2 | Modele d'embedding | **A — all-MiniLM-L6-v2 (23 MB, 384d)** | Meilleur ratio taille/qualite/vitesse pour V1. 23 MB ONNX quantized (app ~200→~295 MB vs ~400 MB avec nomic). Qualite FR correcte. Migration vers nomic-embed-text-v1.5 possible en V2 (re-embedding batch). WASM portable, zero deps natives supplementaires. |
| Q3 | Granularite d'indexation | **A — Chaque message individuellement** | Plus flexible pour la recherche (retrouver un message precis). Chunking 1000 chars + overlap 200 gere les messages longs. Les paires user+assistant perdraient la granularite de filtrage par role. |
| Q4 | Score threshold | **B — 0.35 (equilibre)** | 0.25 ramenerait trop de bruit, 0.50 serait trop strict. 0.35 = sweet spot pour all-MiniLM-L6-v2 sur des phrases conversationnelles. Non ajustable en V1 (garder simple). |
| Q5 | Scope memoire | **B — Filtrable par projet** | Pattern existant : `getEnabledMcpServers(projectId)` filtre `projectId IS NULL OR projectId = passed`. Meme logique pour le recall. `projectId` deja dans le payload Qdrant (section 6). Filtre natif via `must` conditions — zero overhead. |
| Q6 | Active par defaut | **A — Oui** | La valeur de la feature est dans l'automatisme. Pattern existant : `searchEnabled` dans settings.store.ts. Ajout de `semanticMemoryEnabled: true` + cle dans `ALLOWED_SETTING_KEYS`. Cout RAM/CPU faible (~100 MB, ~5-15ms/embed). |
| Q7 | ONNX Runtime backend | **A — WASM (portable)** | Cross-platform sans compilation native. Pas de deps binaires supplementaires (deja better-sqlite3 comme seul native module). Performance suffisante : ~5-15ms/phrase WASM vs ~2-5ms natif. `@huggingface/transformers` utilise WASM par defaut — zero config. |
| Q8 | Recalls visibles | **B — Badge discret dans ContextWindowIndicator** | Pattern existant : Remote badge (Telegram/WebSocket) dans ContextWindowIndicator. Style pill arrondie `🧠 5 souvenirs` + tooltip au hover. Pas de bloc collapsible (trop intrusif). Contenu des recalls invisible pour l'utilisateur, visible pour le LLM uniquement. |
| Q9 | Embedding multilingue | **A — all-MiniLM-L6-v2 pour V1** | FR acceptable (pas optimal). paraphrase-multilingual = 117 MB vs 23 MB (impact taille x5). Cas d'usage principal = code + tech (souvent anglais ou mixte FR/EN). Migration possible en V2 si rappel FR insuffisant. |
