...
```typescript
// Quand la conversation a un referentiel actif (sticky) :
const library = getLibrary(conversation.activeLibraryId)
...
// V1 : mono-referentiel, query directe (pas de boucle multi-collection)
const results = await queryLibraryCollection(library.id, queryVector, { topK: 10 })
```
