...
Permettre a l'utilisateur de creer des **referentiels documentaires** (= "libraries") qu'il alimente avec ses propres fichiers. Un referentiel est **attachable a une conversation** (1 seul, sticky — il reste actif jusqu'au retrait). Le LLM effectue une recherche semantique dans le referentiel actif a chaque message pour fonder ses reponses sur les documents sources, avec **citation des sources** (fichier, section, ligne) generees automatiquement par le renderer.
...
- **Mono-attach** : attacher 1 referentiel a une conversation (sticky — reste actif jusqu'au retrait)
...
| Injection | Automatique (chaque message) | Sticky (referentiel attache a la conversation) |
| Scope | Global / par projet | Par referentiel, 1 par conversation (sticky) |
...
5. Extraction texte
   ...
   │  - .docx → mammoth.convertToMarkdown() (preserve headings pour citations) (B1)
   │  - .md/.txt/.code → lecture directe
   │  NB : mammoth.extractRawText() reste utilise pour les attachments chat,
   │       mais pour les referentiels on utilise convertToMarkdown() pour
   │       preserver la structure des titres (heading extraction)
   │
...
const CHUNKING_CONFIGS: Record<string, ChunkingConfig> = {
  ...
  csv: {
    ...
    prependHeader: true    // Le header CSV est prepende a chaque chunk (C8)
  }
}
```
...
**Limitation v1 (C5)** : l'extraction de headings ne fonctionne que pour les fichiers Markdown. Pour les PDF, code source et CSV, `heading` sera `null`. Les citations affichent alors uniquement le filename + numeros de lignes.

**CSV — Header prepende (C8)** : pour les fichiers CSV, la premiere ligne (header) est automatiquement prepende a chaque chunk afin de conserver le contexte des noms de colonnes. Le header ne compte pas dans le `chunkSize`.
...
Le retrieval se declenche **automatiquement a chaque message** tant qu'un referentiel est attache a la conversation. Le referentiel est **sticky** : une fois attache, il reste actif pour toute la conversation jusqu'a ce que l'utilisateur le retire explicitement.

**Stockage** : `activeLibraryId` dans la table `conversations` (SQLite), persiste entre sessions.

**Migration** : `ALTER TABLE conversations ADD COLUMN active_library_id TEXT REFERENCES libraries(id)`
...
// 1. Verifier si un referentiel est attache a la conversation (sticky)
const activeLibraryId = conversation.activeLibraryId
if (activeLibraryId) {
  // 2. Embed la question utilisateur avec le modele du referentiel
  const library = getLibrary(activeLibraryId)
  const queryVector = await embedForLibrary(content, library.embeddingModel)

  // 3. Query la collection Qdrant du referentiel
  const results = await queryLibraryCollection(activeLibraryId, queryVector, {
    topK: 10,                    // Top 10 chunks
    scoreThreshold: 0.30         // Seuil de pertinence
  })

  // 4. Selection avec budget token
  const selectedChunks = selectBestChunks(results, {
    maxChunks: 10,                // Max 10 chunks
  ...
  // 7. Stocker les sources injectees dans contentData du message assistant
  // (pour generer la section "Sources utilisees" automatique)
}
```
...
### 8.4 Section "Sources utilisees" automatique (C1)

En bas de chaque message assistant qui utilise un referentiel, une section **"Sources utilisees"** est generee **automatiquement par le renderer** a partir des chunks injectes dans `<library-context>` — **pas basee sur ce que le LLM cite** (qui peut oublier ou halluciner des references).
...
retourner une fonction de nettoyage [source:1].

──────────────
📚 Sources utilisees (Doc React 19) :
  [1] react-hooks.md > useEffect (l.45-68)
  [2] api-reference.md > Hooks API (l.120-145)
  [3] migration-guide.pdf (p.12)
```

**Fiabilite** : cette section est deterministe — elle liste exactement les chunks qui ont ete injectes dans le prompt, pas ceux que le LLM pretend avoir utilises.

### 8.5 Metadata des sources dans le message

Les sources injectees sont stockees dans `contentData` du message assistant :
...
contentData: {
  librarySources?: Array<{
    ...
    heading: string | null  // "useEffect" (null pour PDF/code/CSV)
    lineStart: number | null
    lineEnd: number | null
    chunkPreview: string    // Premiers 200 chars du chunk
    score: number           // Score de similarite Qdrant
  }>
}
```

Et dans `contentData` du message utilisateur, on stocke le referentiel actif :

```typescript
contentData: {
  libraryId?: string        // "lib-abc" (mono-referentiel v1)
}
```
...
// ── Conversation — attach/detach referentiel ────

ipcMain.handle('library:attach', async (_, { conversationId, libraryId }) => void)
  // SET conversations.active_library_id = libraryId

ipcMain.handle('library:detach', async (_, { conversationId }) => void)
  // SET conversations.active_library_id = NULL
```

### 9.1bis Progress IPC (A6)

L'indexation envoie des events de progression au renderer :

```typescript
// Channel : 'library:indexing-progress'
// Direction : main → renderer (webContents.send)

interface LibraryIndexingProgress {
  libraryId: string
  sourceId: string
...
// Usage dans LibraryService :
mainWindow.webContents.send('library:indexing-progress', {
  libraryId, sourceId, percent: 45, status: 'embedding'
})
```
...
```typescript
// sendMessageSchema — ajouter libraryId directement au schema existant :
libraryId: z.string().optional()
  // ID du referentiel actif pour cette conversation (sticky)
  // NB : pas de schema "extended" — on ajoute le champ au schema existant
```
...
window.api = {
  ...
  onLibraryIndexingProgress: (cb: (progress: LibraryIndexingProgress) => void) => {
    ipcRenderer.on('library:indexing-progress', (_, progress) => cb(progress))
  },
...
### 10.4 InputZone — Attacher un referentiel (sticky)

Un nouveau bouton dans la barre d'actions de l'InputZone (a cote des attachments, roles, prompts).

**V1 : 1 seul referentiel par conversation** — le LibraryPicker est un **select simple** (pas checkboxes multi-select).

Le referentiel est **sticky** : une fois attache, il reste actif pour toute la conversation. Un badge permanent indique le referentiel actif au-dessus de l'input.

```
┌─ InputZone ──────────────────────────────────────┐
│                                                   │
│  📚 Referentiel actif : Doc React 19 [×]         │  ← Badge permanent sticky
│                                                   │
│ [📎] [📚] [🎭] [⚡] [🔍]           [Envoyer]    │
│       ↑                                           │
│  Library Picker (select simple)                   │
│  ┌────────────────────────────┐                   │
│  │ Doc React 19  (847 ch.)   │                   │
...
Le clic sur [×] du badge detache le referentiel (library:detach).
```
...
### 10.6 MessageItem — Citations inline + Sources automatiques

Dans le rendu des messages assistant, les `[source:N]` sont transformes en badges cliquables.

**Section "Sources utilisees" automatique** (C1) : en bas du message, une section est generee **par le renderer** a partir des chunks injectes (stockes dans `contentData.librarySources`). Cette section est **deterministe** — elle liste exactement les chunks qui ont ete fournis au LLM, independamment de ce que le LLM cite dans sa reponse.
...
**Note** : les headings peuvent etre `null` pour les fichiers non-Markdown (PDF, code, CSV). Dans ce cas, seuls le filename et les numeros de lignes sont affiches.
...
### 11.3 Selection des chunks — Algorithme (v1 mono-referentiel)

```
1. Query Qdrant top-10 dans la collection du referentiel actif
   - Filtrer score < minScore

2. Selection gloutonne avec budget :
...
**Note v1** : avec un seul referentiel, pas besoin de `diversityWeight` ni de fusion multi-collection. L'algorithme est simplifie.
...
- **V1 mono-ref** : query unique sur 1 collection (pas de `Promise.all()` necessaire)
...
5. **Handlers IPC** : `src/main/ipc/library.ipc.ts` (14 handlers dont attach/detach)
6. **Validation Zod** pour chaque handler
7. **Preload bridge** : 16 nouvelles methodes (dont progress listener)
8. **Extension `chat.ipc.ts`** :
   - Ajout `libraryId` (string, pas array) dans `sendMessageSchema`
   - Retrieval sticky (via `conversation.activeLibraryId`) + injection `<library-context>` avant `streamText()`
   - Stockage `librarySources` dans `contentData` du message assistant
9. **Extension StreamChunk** : `librarySourcesCount`
10. **Migration** : `ALTER TABLE conversations ADD COLUMN active_library_id`
...
### Cleanup / Factory Reset (A1)

**Ordre FK strict** pour les 3 nouvelles tables :
1. `library_chunks` (depend de `library_sources` et `libraries`)
2. `library_sources` (depend de `libraries`)
3. `libraries` (depend de `projects`)

**A integrer dans** :
- `data.ipc.ts` cleanup zone orange : supprimer les 3 tables dans l'ordre ci-dessus
- `data.ipc.ts` factory reset zone rouge : idem
- **Drop collections Qdrant** : boucle sur toutes les collections `library_*` + suppression via `DELETE /collections/library_{id}`
- `conversations` : `ALTER TABLE` ajoute `active_library_id` → le cleanup des conversations reset cette colonne aussi
- Fichiers : `trash` du dossier `userData/libraries/` (pas `rm`)
...
### D3 : Injection sticky par conversation (vs par message)

**Choix : l'utilisateur attache un referentiel a la conversation — il reste actif (sticky) jusqu'au retrait explicite.**

- (+) Zero friction apres le premier attach — pas de clic a chaque message
- (+) Coherence des citations tout au long de la conversation
- (+) L'instruction de citer les sources est presente a chaque echange
- (+) L'utilisateur controle exactement quel contexte le LLM utilise
- (-) Consomme des tokens a chaque message tant que le referentiel est attache
- **Stockage** : `conversations.active_library_id` (SQLite) — persiste entre sessions
...
### D7 : Persistance du referentiel actif dans l'historique

**Choix : stocker `libraryId` dans `contentData` du message utilisateur + `activeLibraryId` dans la table `conversations`.**

- `contentData.libraryId` : trace quel referentiel etait actif au moment de l'envoi
- `conversations.active_library_id` : referentiel sticky actuellement attache
- Pas de re-query automatique des anciens messages — les chunks injectes sont deja dans l'historique
- V1 : mono-referentiel (`string`, pas `string[]`)
...
- **Export/Import .mlx** : le format d'export securise n'inclut pas les referentiels (v1)
- **Remote Telegram/Web** : pas d'interaction avec les referentiels
- **CommandPalette** (Cmd+K) : pas de recherche dans les referentiels (v1)
- **FTS5** : la recherche full-text existante ne couvre pas les referentiels (recherche semantique uniquement)
...
| `src/main/ipc/library.ipc.ts` | 14 handlers IPC + validation Zod (dont attach/detach) |
...
|---|---|
| `src/main/db/schema.ts` | Ajouter tables `libraries`, `library_sources`, `library_chunks` + `active_library_id` sur `conversations` |
| `src/main/ipc/index.ts` | Enregistrer handlers library, ajouter settings keys |
| `src/main/ipc/chat.ipc.ts` | Ajouter `libraryId` au schema, retrieval sticky + injection `<library-context>` |
| `src/preload/index.ts` | Ajouter 16 methodes library (dont attach/detach/progress) |
...
// Extension sendMessageSchema — ajouter directement au schema existant (pas de schema "extended")
// Dans chat.ipc.ts, ajouter ce champ au sendMessageSchema existant :
// libraryId: z.string().optional()   // Referentiel actif (mono-ref v1, sticky)
```
